...
def one(mod: Dict):
    ...
    output_folder_path = Path(config.get_string(GENERATED_MODS_OUTPUT_FOLDER))
    ...
    if cmd_exists(unreal_pack_cmd := config.get_string(UNREAL_PACK_COMMAND)):
        ...
        pak_file_path = output_folder_path / (mod_name + f" {mod[Mod.VERSION_NAME]}_P.pak")
        plugin_file_name = f"{mod_name}.{PLUGIN_FILE_EXTENSION}"
        plugin_file_path = output_folder_path / plugin_file_name
        zip_archive_file_path = Path(config.get_string(GENERATED_MODS_OUTPUT_FOLDER)) / (mod_name + f" {mod[Mod.VERSION_NAME]}.zip")

        # Write definition file for new master version if it was just updated
        ...
        with ZipFile(zip_archive_file_path, 'w') as zipObj:
            zipObj.write(pak_file_path, pak_file_path.relative_to(output_folder_path))

        # Cleanup pakiing config.txt file and generated .pak file once zipped
        pak_config_file_path.unlink(missing_ok=True)
        pak_file_path.unlink(missing_ok=True)

        if not zip_archive_file_path.is_file():
            trigger_error(m.E_FAILED_CREATE_ZIP, halt=False)
            print(stdout_to_output(proc.stdout))
            return
